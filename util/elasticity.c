...
void get_boundary_nodes (size_t** bnd_nTags, size_t* bnd_nTags_n, size_t** sym_nTags, size_t* sym_nTags_n){
  ...
  size_t dimTags_n;
  *sym_nTags_n = 0; // if no symmetry --vlepere
  ...
  for (size_t i=0;i<dimTags_n;i+=2){  
    ...
    if (strcmp(name,"symmetry") == 0){
      gmshModelMeshGetNodesForPhysicalGroup(dimTags[i], dimTags[i+1], sym_nTags, sym_nTags_n, &cc, &cc_n, &ierr);
      printf("Info    : Symmetry found physical group %d\n",dimTags[i+1]);
    }
  ...
  for (size_t i=0; i<*sym_nTags_n; i++) (*sym_nTags)[i]--;
...
int assemble_system(Matrix** K, Matrix** M, double** coord, size_t** boundary_nodes, size_t* n_boundary_nodes, size_t **symmetry_nodes, size_t* n_symmetry_nodes, double E, double nu, double rho){
  ...
  get_boundary_nodes(boundary_nodes, n_boundary_nodes, symmetry_nodes, n_symmetry_nodes); // the position in the matrix is at bnd_nodes[i]-1
